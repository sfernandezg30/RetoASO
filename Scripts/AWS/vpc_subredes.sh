...
# Fecha: 02-02-2024
# Creación de VCP y subredes

# Crear la VPC
vpc_id=$(aws ec2 create-vpc --cidr-block 10.100.0.0/16 --tag-specifications 'ResourceType=vpc,Tags=[{Key=Name,Value=VPC-SALMONES}]' --output json | jq -r '.Vpc.VpcId')
